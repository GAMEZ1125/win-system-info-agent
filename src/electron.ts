...
function setupIPC() {
  ...
  logger.info('Configuración IPC general inicializada');
}
...
function createWindow() {
          ...
          <div id="status-tab" class="tab-content">
            ...
            <div class="dashboard-grid">
              <div class="card">
                ...
                <div id="serviceStatus2" class="status-value ${
                  serviceStatus === 'running' ? 'running' : 
                  serviceStatus === 'stopped' ? 'stopped' : 'unknown'
                }">
                  <span class="status-icon">
  ...
  mainWindow.webContents.on('did-fail-load', (event, errorCode, errorDescription) => {
    logger.error(`Error al cargar ventana: ${errorDescription} (${errorCode})`);
  });
}
...
function setupCollectInfoHandler() {
  ...
  try {
    ipcMain.removeHandler('collect-system-info');
  } catch (error) {
    // Si no existía un manejador, esto podría lanzar un error que ignoramos
    logger.debug('No había manejador previo para eliminar');
  }
  ...
  logger.info('Manejador collect-system-info registrado correctamente');
}

// Modificar la función showToastNotification para que funcione con la nueva UI
function showToastNotification(title: string, message: string, type: string = 'info') {
  // Registrar en logs
