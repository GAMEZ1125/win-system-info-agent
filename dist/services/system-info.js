...
async function getSystemInfo() {
    try {
        logger_1.logger.info('Collecting system information');
        ...
        try {
            ...
            manufacturer = (0, child_process_1.execSync)('wmic computersystem get manufacturer')
                .toString()
                .split('\n')[1]
                .trim();
            // Obtener modelo del dispositivo
            model = (0, child_process_1.execSync)('wmic computersystem get model')
                .toString()
                .split('\n')[1]
                .trim();
            // Obtener número de serie
            serialNumber = (0, child_process_1.execSync)('wmic bios get serialnumber')
                .toString()
                .split('\n')[1]
                .trim();
            // Intentar obtener clave de producto de Windows (puede requerir elevación)
            try {
                osProductKey = (0, child_process_1.execSync)('wmic path softwarelicensingservice get OA3xOriginalProductKey')
                    .toString()
                    .split('\n')[1]
                    .trim();
            }
            catch (error) {
                logger_1.logger.warn('Could not retrieve OS product key (may require administrator privileges)');
            }
            // Obtener service pack
            const osInfoOutput = (0, child_process_1.execSync)('wmic os get ServicePackMajorVersion')
                .toString()
                .split('\n')[1]
                .trim();
            osServicePack = osInfoOutput !== '' ? `SP${osInfoOutput}` : 'None';
            // Determinar tipo de dispositivo
            const chassisType = (0, child_process_1.execSync)('wmic systemenclosure get chassistypes')
                .toString()
                .split('\n')[1]
                .trim();
            ...
            logger_1.logger.error(`Error collecting WMI information: ${error}`);
        }
        ...
        try {
            const driveData = (0, child_process_1.execSync)('wmic logicaldisk where DeviceID="C:" get Size,FreeSpace')
                .toString()
                .trim()
                .split('\n')[1]
                .trim()
            ...
            logger_1.logger.error(`Error collecting disk information: ${error}`);
        }
        ...
        logger_1.logger.error(`Error in getSystemInfo: ${error}`);
        throw error;
    }
}
...
function getIpAddress() {
    try {
        ...
        logger_1.logger.error(`Error getting IP address: ${error}`);
        return 'Unknown';
    }
}
function getMacAddress() {
    try {
        ...
        logger_1.logger.error(`Error getting MAC address: ${error}`);
        return 'Unknown';
    }
}
